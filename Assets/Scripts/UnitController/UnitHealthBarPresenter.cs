...
namespace Strategy.Units
{
    ...
    public class UnitHealthBarPresenter : MonoBehaviour
    {
        ...
        [SerializeField] private bool _lockWorldRotation;
        ...
        {
            if (_combat != null)
                _combat.HealthChanged += OnHealthChanged;
        ...
        {
            if (_lockWorldRotation && _barRoot != null && IsVisible)
                _barRoot.rotation = Quaternion.Euler(_worldEulerAngles);
        }

        private void OnHealthChanged(UnitCombat combat)
        {
        ...
        {
            if (_combat == null || _combat.MaxHealth <= 0f)
                return 0f;

            if (UnitHealthBarVisibility.ForceVisible)
                return 1f;
        ...
        {
            if (_combat == null || _fill == null)
                return;

            _lastFillAmount = _combat.NormalizedHealth;
